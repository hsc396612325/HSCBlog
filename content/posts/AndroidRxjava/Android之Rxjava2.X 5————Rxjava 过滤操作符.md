---
title: "Android之Rxjava2.X 5————Rxjava 过滤操作符"
date: 2019-04-05T22:40:54+08:00
draft: false
...
#### 2.类型
![这里写图片描述](/image/Android_Rxjava/7_0.png)
### 三. 根据指定事件条件过滤事件
#### 1. filter()
* 作用：通过一定逻辑来过滤被观察者发送的事件，如果返回true则发送事件，否则不会发送 
* 应用场景：筛选符合要求的事件
原理图：
![这里写图片描述](/image/Android_Rxjava/7_1.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_2.png)
#### 2.ofType()
* 作用：ofType是filter操作符的一个特殊形式。它过滤一个Observable只返回指定类型的数据。
原理图：
![这里写图片描述](/image/Android_Rxjava/7_3.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_4.png)

#### 3.skip() & skipLast()
* skip()作用：忽略Observable'发射的前N项数据，只保留之后的数据。
*  skipLast()作用:从结尾往前数跳过制定数量的事件 
原理图：
![这里写图片描述](/image/Android_Rxjava/7_5.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_6.png)
```java
Observable.just(1, 2, 3,4)
                .skipLast(2)
                .subscribe(new Observer<Integer>() {
...
```
![这里写图片描述](/image/Android_Rxjava/7_7.png)
#### 4.distinct（） / distinctUntilChanged（）
* 作用：过滤事件序列中重复的事件 / 连续重复的事件

原理图：
![这里写图片描述](/image/Android_Rxjava/7_8.png)
![这里写图片描述](/image/Android_Rxjava/7_9.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_10.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_11.png?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM4NDk5ODU5/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
#### 2.takeLast（）
* 作用:发射Observable发射的最后N项数据

原理图：
![这里写图片描述](/image/Android_Rxjava/7_12.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_13.png)
### 五. 根据指定时间过滤事件
...
```
![这里写图片描述](/image/Android_Rxjava/7_14.png)
```java
  Observable.create(new ObservableOnSubscribe<Integer>() {
            @Override
            public void subscribe(ObservableEmitter<Integer> e) throws Exception {
...
```
![这里写图片描述](/image/Android_Rxjava/7_15.png)
#### 2.Sample（）
...
```
![这里写图片描述](/image/Android_Rxjava/7_16.png)
### 六. 根据指定事件位置过滤事件
...
```
![这里写图片描述](/image/Android_Rxjava/7_17.png)
...
```
![这里写图片描述](/image/Android_Rxjava/7_18.png)
#### 3.elementAtOrError（）
* 作用:在elementAt（）的基础上，当出现越界情况（即获取的位置索引 ＞ 发送事件序列长度）时，即抛出异常
 ...
 ```
 ![这里写图片描述](/image/Android_Rxjava/7_19.png?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM4NDk5ODU5/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
### 七.参考资料
